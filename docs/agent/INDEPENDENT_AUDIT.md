...
- **Low / Documented Limitations:**
  1. *Fictional Actuarial Tables:* Rate tables in `packages/rules/src/v1.ts` are simplified demonstration multipliers and must be replaced with client actuarial tables in production.
  2. *Single-Node Local Persistence Default:* Local in-memory session store is used by default for zero-credential simplicity; production requires PostgreSQL or AWS RDS with encryption at rest.
