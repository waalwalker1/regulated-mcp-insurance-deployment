...
- **Sample Defense:**
  - *Interviewer:* "What prevents an LLM prompt injection from issuing a €0 quote?"
  - *FDE Response:* "Input sanitizers flag injection patterns, but fundamentally, the pricing engine is a pure server-side function (`calculatePricing`). The client payload does not accept premium overrides."
