...
import { QuoteInputSchema, type QuoteInput } from '../packages/domain/src/schemas.js';
...
describe('Property-Based Testing (fast-check)', () => {
  const quoteInputArbitrary = fc.record({
    ...
    propertyType: fc.constantFrom('apartment', 'detached_house', 'semi_detached', 'terraced_house', 'villa' as const),
    occupancyType: fc.constantFrom('owner_occupied', 'tenant', 'landlord' as const),
    constructionYearBand: fc.constantFrom('pre_1970', '1970_1999', '2000_2015', 'post_2015' as const),
    floorAreaBand: fc.constantFrom('under_50_sqm', '50_100_sqm', '101_150_sqm', '151_250_sqm', 'over_250_sqm' as const),
  ...
  it('property: positive premium invariant — net premium is always >= €50 and total > 0', () => {
    fc.assert(
      fc.property(quoteInputArbitrary, (input) => {
        ...
        expect(pricing.totalAnnualPremium).toBeGreaterThan(pricing.netAnnualPremium);
