...
describe("Domain Schemas & Validation", () => {
  ...
  it("rejects invalid country or bad postcode format", () => {
    ...
    expect(() => QuoteInputSchema.parse(invalid)).toThrow(
  ...
  it("enforces explicit consent true", () => {
    ...
    expect(
      ConsentDeclarationSchema.parse(validConsent).hasConsentedToDataProcessing,
    ).toBe(true);
...
describe("Funnel State Machine", () => {
  ...
  it("allows valid step transitions", () => {
    ...
    expect(
      FunnelStateMachine.canTransition(session.step, "COLLECTING_PROPERTY"),
    ).toBe(true);
  ...
  it("throws on invalid step jumps", () => {
    const session = createTestSession();
    expect(() => FunnelStateMachine.transition(session, "QUOTED")).toThrow(
      /Cannot transition funnel session/,
    );
  });
