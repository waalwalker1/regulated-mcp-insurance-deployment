...
describe("Rule Versioning and Quote Replay Reproducibility", () => {
  ...
  it("preserves exact quote replay when re-evaluated against original rule version", () => {
    ...
    const originalHash = computeQuoteHash(
      RULE_SET_V1.version,
      input,
      originalPricing,
    );
    ...
    expect(v2Pricing.totalAnnualPremium).not.toBe(
      originalPricing.totalAnnualPremium,
    );
    ...
    const replayedHash = computeQuoteHash(
      RULE_SET_V1.version,
      input,
      replayedPricing,
    );

    expect(replayedPricing.totalAnnualPremium).toBe(
      originalPricing.totalAnnualPremium,
    );
    expect(replayedPricing.netAnnualPremium).toBe(
      originalPricing.netAnnualPremium,
    );
