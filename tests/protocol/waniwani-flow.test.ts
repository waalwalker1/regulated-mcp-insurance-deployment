...
describe("Waniwani SDK Typed Flow Protocol Tests", () => {
  it("should execute end-to-end multi-step funnel with interrupts, confirmation, consent, and deterministic quote", async () => {
    ...
    const step1 = await harness.start(
      "I want home insurance for my apartment in Paris",
    );
    expect(step1.status).toBe("interrupt");
    if (step1.status === "interrupt") {
      const hasCountry =
        step1.field === "country" ||
        step1.questions?.some((q) => q.field === "country");
    ...
    if (step3.status === "interrupt") {
      const hasTier =
        step3.field === "coverageTier" ||
        step3.questions?.some((q) => q.field === "coverageTier");
    ...
    if (step4.status === "interrupt") {
      const hasConfirm =
        step4.field === "parametersConfirmed" ||
        step4.questions?.some((q) => q.field === "parametersConfirmed");
    ...
    if (step5.status === "interrupt") {
      const hasConsent =
        step5.field === "hasConsented" ||
        step5.questions?.some((q) => q.field === "hasConsented");
