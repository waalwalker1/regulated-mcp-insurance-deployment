...
export const OccupancyTypeSchema = z.enum(['owner_occupied', 'tenant', 'landlord'], {
 ...
 */
export const QuoteInputSchema = BaseQuoteInputSchema.strict().superRefine((data, ctx) => {
...
export const ConsentDeclarationSchema = z.object({
  hasConsentedToDataProcessing: z.literal(true, {
    errorMap: () => ({ message: 'User must explicitly confirm consent to process quote data.' })
  }),
 ...
 */
export const EligibilityStatusSchema = z.enum(['eligible', 'referral_required', 'declined']);
