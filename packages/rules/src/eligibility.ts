...
export function evaluateEligibility(
  ...
  if (input.propertyType === 'villa' && input.floorAreaBand === 'over_250_sqm' && input.claimsCount5Years >= 2) {
  ...
  if (reasonCodes.length === 0) {
    return {
      ...
      explanation: 'Property meets all standard underwriting guidelines for automated quotation.',
      evaluatedAt,
  ...
  return {
    ...
    isEligible: false,
    reasonCodes,
    explanation: 'Risk characteristics require manual review by a licensed underwriting specialist before binding.',
    evaluatedAt,
    ruleVersion: ruleSet.version
  };
}
