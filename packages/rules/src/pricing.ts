...
export function calculatePricing(
  ...
  const propertyTypeMultiplier = ruleSet.propertyMultipliers[input.propertyType] ?? 1.0;
  const occupancyMultiplier = ruleSet.occupancyMultipliers[input.occupancyType] ?? 1.0;
  const areaMultiplier = ruleSet.areaMultipliers[input.floorAreaBand] ?? 1.0;
  const constructionYearMultiplier = ruleSet.constructionYearMultipliers[input.constructionYearBand] ?? 1.0;
  const claimsMultiplier = ruleSet.claimsMultipliers[Math.min(input.claimsCount5Years, 3)] ?? 1.0;
  const coverageTierMultiplier = ruleSet.coverageTierMultipliers[input.coverageTier] ?? 1.0;
  const deductibleDiscount = ruleSet.deductibleDiscounts[input.deductible] ?? 0.0;

  // Multiply base premium through risk factors
  const grossAnnual = baseAnnualPremium *
  ...
  const netAnnualPremium = Math.max(50.0, Math.round((grossAnnual - deductibleDiscount) * 100) / 100);

  const taxRatePercent = (ruleSet.taxRates[input.country] ?? 0.18) * 100;
  const fictionalTaxAmount = Math.round((netAnnualPremium * (taxRatePercent / 100)) * 100) / 100;
  const totalAnnualPremium = Math.round((netAnnualPremium + fictionalTaxAmount) * 100) / 100;
