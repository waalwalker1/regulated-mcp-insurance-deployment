...
export function getRuleSet(version: string = DEFAULT_RULE_VERSION): InsuranceRuleSet {
  const ruleSet = RULE_REGISTRY[version];
  if (!ruleSet) {
    throw new Error(`Unknown insurance rule version '${version}'. Available: ${Object.keys(RULE_REGISTRY).join(', ')}`);
  }
  return ruleSet;
}
