# Third-Party Notices & Attribution

This repository incorporates open-source libraries under permissive licenses:

---

## 1. @modelcontextprotocol/sdk

- **License:** MIT License
- **Copyright:** (c) Anthropic PBC / Model Context Protocol Contributors
- **URL:** https://github.com/modelcontextprotocol/typescript-sdk

---

## 2. @waniwani/sdk

- **License:** MIT License
- **Copyright:** (c) 2026 Waniwani AI
- **URL:** https://github.com/WaniWani-AI/sdk

---

## 3. Zod

- **License:** MIT License
- **Copyright:** (c) 2020 Colin McDonnell
- **URL:** https://github.com/colinhacks/zod

---

## 4. Fastify

- **License:** MIT License
- **Copyright:** (c) 2016-present Fastify Contributors
- **URL:** https://github.com/fastify/fastify

---

## Non-Affiliation Statement

Northstar Home Insurance EU is a fictional reference domain used by this independent open-source implementation. The project is not affiliated with, endorsed by, or sponsored by Waniwani AI, Anthropic, or any commercial insurer.
