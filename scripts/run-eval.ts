...
async function runEvaluationSuite() {
  ...
  // Scenario 1: Standard France Apartment (Owner Occupied)
  {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        quote.pricing.totalAnnualPremium === 161.66 &&
        quote.status === "active",
  ...
  // Scenario 2: Spain Detached House (Owner Occupied)
  {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        quote.pricing.totalAnnualPremium > 0 && quote.input.country === "ES",
  ...
  // Scenario 3: Portugal Tenant Apartment
  {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        quote.pricing.totalAnnualPremium > 0 &&
        quote.input.occupancyType === "tenant",
  ...
  // Scenario 4: Germany Terraced House (Landlord)
  {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        quote.pricing.taxRatePercent === 19 && quote.input.country === "DE",
  ...
  // Scenario 5: Italy Villa (Owner Occupied, 0 claims)
  {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        quote.pricing.taxRatePercent === 21 &&
        quote.pricing.propertyTypeMultiplier === 1.6,
  ...
  // Scenario 6: Referral on Excessive Claims (>3 claims)
  {
    ...
    results.push({
      ...
      passed: Boolean(
        evaluated.step === "REFERRED" &&
        evaluated.eligibilityResult?.isEligible === false &&
        evaluated.eligibilityResult?.reasonCodes.includes(
          "CLAIMS_THRESHOLD_EXCEEDED",
        ),
      ),
  ...
  // Scenario 7: Referral on Large Villa with High Loss History
  {
    ...
    results.push({
      ...
      passed: Boolean(
        evaluated.step === "REFERRED" &&
        evaluated.eligibilityResult?.reasonCodes.includes(
          "HIGH_VALUE_HIGH_CLAIMS_REFERRAL",
        ),
      ),
  ...
  // Scenario 8: Consent Gating Enforcement (Attempt calculation without consent)
  {
    ...
    results.push({
      ...
      actual: rejected
        ? "Server threw [CONSENT_REQUIRED] exception"
        : "Server failed to enforce consent gate",
      latencyMs: Date.now() - t0,
    });
  }
  ...
  // Scenario 9: Prompt Injection in Postcode
  {
    ...
    results.push({
      ...
      actual: caughtTampering
        ? "Blocked and security audit event logged"
  ...
  // Scenario 10: State Correction Loop (Invalidate downstream quote)
  {
    ...
    const sCorrected = await engine.correctField(session.sessionId, {
      propertyType: "villa",
    });
    ...
    results.push({
      ...
      passed:
        sCorrected.activeQuote === undefined &&
        sCorrected.step === "COLLECTING_PROPERTY" &&
        sCorrected.correctionCount === 1,
      expected:
        "Active quote invalidated, step reverted to COLLECTING_PROPERTY, correction count incremented",
  ...
  // Scenario 11: Dynamic Quote Deductible Adjustment Loop
  {
    ...
    const qAdjusted = await engine.adjustQuote(session.sessionId, {
      deductible: 1000,
    });
    results.push({
      id: "SCN-011",
      ...
      category: "STATE_CORRECTION",
      passed:
        qAdjusted.status === "adjusted" &&
        qAdjusted.pricing.totalAnnualPremium < q1.pricing.totalAnnualPremium,
  ...
  // Scenario 12: Rule Versioning Deterministic Replay
  {
    ...
    const replayedHashV1 = computeQuoteHash(
      RULE_SET_V1.version,
      input,
      replayedV1,
    );
    ...
    results.push({
      ...
      passed:
        hashV1 === replayedHashV1 &&
        hashV1 !== hashV2 &&
        pV1.totalAnnualPremium === replayedV1.totalAnnualPremium,
      expected:
        "Replayed quote under v1 rules matches original hash and price perfectly",
  ...
  // Scenario 13: Cryptographic Audit Hash Chain Verification
  {
    ...
    const verification = await auditStore.verifyChainIntegrity(
      session.sessionId,
    );
    results.push({
      id: "SCN-013",
      ...
      expected:
        "Unbroken SHA-256 hash sequence verified across all lifecycle events",
  ...
  for (const combo of combinations) {
    ...
    results.push({
      ...
      category: "HAPPY_PATH",
      passed:
        pricing.totalAnnualPremium > 0 && !isNaN(pricing.totalAnnualPremium),
  ...
  console.log(
    `  EVALUATION SUMMARY: ${passedCount}/${results.length} Scenarios Passed (${failedCount} Failed)`,
  );
  ...
  for (const r of results) {
    console.log(
      `  ${r.passed ? "✓" : "✗"} [${r.id}] ${r.name.padEnd(55)} (${r.latencyMs}ms)`,
    );
  }
